...
async function _subsToHub({provider, value}) {
  ...
  var createdHub = value['data']['onCreateHub']
  console.log(createdHub);
}
function unsubsToHub() {
  subsHub.unsubscribe()
...
async function createHubEntry(){
  // following block is to make a fake entry of a hub
  ////////////////////////////////////////////////
  // this uuid could be of any hub
  const hubUuid = "5d4227ec-0dc0-4b61-a0bd-42373c064fde"
  const response = await API.graphql({
    query: queries.getHub,
    variables: {
     uuid: hubUuid
    }
  })
  const newHubEntry = response['data']['getHub']
  ////////////////////////////////////////////////
  const newHubDetails = {
    uuid: `subsTesting ${uuidv4()}`,
    name: `subsTesting newHubEntry['name']}`,
    hubUniqueId: newHubEntry['hubUniqueId'],
    hubCompartmentId: newHubEntry['hubCompartmentId'],
    description: newHubEntry['description']
  }
